...
| 8 | `P2-SUX-001-20` | `done` | `P2` | First-final verification: walk every route family at 360 / 768 / 1280 / 1440 px and confirm the canonical header shape, avatar presence, and bottom / top nav behaviour (subtasks `-01` through `-19`) |
...
**Third wave — Account-family pack (subtasks `-21` through `-28`) — closed 2026-05-26.** The Account-family pack landed end-to-end. The canonical Account-family IA ADR ([account-family-shape-v1](../architecture/account-family-shape-v1.md), `-21`) ratified four routes / one job each, with role gating applied at the page-body level only — the avatar menu and the `(account)/layout.tsx` nav rail still expose all four entries to every signed-in account. `-22` consolidated notification preferences by relocating `NotificationPreferencesForm` and its Server Action from `(account)/settings/` to a new `Preferences` tab on `(account)/notifications/`; the `Inbox` stays the default tab, the TabBar is a DS `TabBar` in link mode, and the embedded second Panel disappears from `/settings`. `-23` added the static `NOTIFICATION_CATEGORY_AUDIENCE` lookup in `src/modules/notifications/constants.ts`, filtered `notificationCategories` by `hasRole`, switched `NOTIFICATION_CATEGORY_DESCRIPTIONS["lesson_reminders"]` to a per-role record, and added the role-shaped Inbox labels (`getNotificationTypeLabel(type, activeRole)`) — a student-only account now sees two categories, tutor-only sees three, dual-role sees four; rows are still routed by `account.id`. `-24` removed the `Product inbox only` InlineNotice, the `Channel rule` Panel, the `Notification summary` metric grid, and the `Latest product updates` wrapper Panel from `/notifications`; each Inbox row now carries exactly one `StatusBadge` (the type), and unread state moved to a `Card selected` treatment. `-25` rebuilt `/privacy` as a legal-acknowledgement surface — pending banner, current-notice `Panel`, `Policy documents` `Section` with permanent secondary-variant links to `/privacy-policy` / `/terms` / `/trust-and-safety`, and the published-history list — removing the `Privacy surface scope` Panel, the `Current notice state` metric grid, and the `Deferred controls` placeholder. `-26` role-gated `/billing`: tutor-only accounts now see one `Panel` titled `Tutor payouts live in Earnings` with one CTA to `/tutor/earnings` (no payment-history shapes, no `Scope guardrails`, no `InlineNotice`); the student body keeps the three metric Cards with the renamed labels `Total paid` / `Awaiting confirmation` / `Refunds`. `-27` reduced `/settings` to one `Profile` Panel — the `Preferred lesson language` Section was renamed `Lesson language` with a per-role description (`resolveLessonLanguageDescription`), the Email and Timezone Sections now carry `sectionNote` helper copy, the timezone value renders through `getTimezoneLabel(timezone)` instead of the raw IANA identifier, and a single `Manage notification preferences →` link at the Panel footer routes to `/notifications?tab=preferences`. End-to-end verification under `-28` confirmed every binding rule from the ADR holds: `grep` for `Bell inbox`, `Channel rule`, `No chat replay`, `Visible product updates`, `Privacy surface scope`, `Current notice state`, `Deferred controls`, `Scope guardrails`, `Billing history only`, `Shared account route`, `Payer-safe summary`, and `No payout controls here` returns zero hits across `src/app/(account)/**`, `src/components/**`, and `src/modules/**`; `/settings` renders one `Profile` Panel with the `Manage notification preferences →` footer link and no embedded preferences Panel; `/notifications` opens on `Inbox` by default and the `Preferences` tab renders `NotificationPreferencesForm` against the role-filtered category set; `/privacy` carries the `Policy documents` link strip with the three canonical public-policy URLs; `/billing` branches on `hasRole(account, "student")` after `getSharedAccountRouteContext` resolves. `/results` and `/tutor/overview` were re-walked to confirm no regression from the first-wave and second-wave changes (the `-01` through `-13` body invariants and the `-14` through `-20` header invariants all still hold). No new vendors, reference-data tables, or DB columns were introduced; the only new constants-level addition is `NOTIFICATION_CATEGORY_AUDIENCE` plus the per-role `lesson_reminders` description in `src/modules/notifications/constants.ts`. Automated verification: see § 5.28 verification log. Known caveat: the `-21` ADR § 4.5 specifies that a dual-role account on `/billing` should see a small Earnings cross-link `InlineNotice` at the top of the student history; the `-26` scope only addressed the tutor-only branch and the student copy pass, so that cross-link is the one ADR detail not yet wired — a one-line follow-on if dual-role test coverage flags it. `P2-SUX-001` is now closed.

**Second wave — cross-family header pack (subtasks `-14` through `-20`) — closed 2026-05-26.** The header pack landed end-to-end. The canonical `AppHeader` ADR ([app-header-shape-v1](../architecture/app-header-shape-v1.md), `-14`) ratified one header shape across every `AppFrame` consumer; `loadViewerIdentity()` now runs in the `(public)` layout too (`-15`), so the avatar slot has data on every signed-in route family including the public one; the avatar `<Link>` was replaced by the DS `AvatarMenu` popover (`-16`) so account excursions no longer swap the active family's nav rail (and `ViewerIdentity` lost the `settingsHref` field in the same migration); the nav-label / eyebrow / public-nav-trim cleanup (`-16-5`) aligned same-href labels (`Get Matched` / `My matches` / `Find Tutors`) across the public and student families, made the eyebrow optional, dropped the implementation-leaking eyebrows from student / tutor / internal, and moved `How It Works` / `Trust & Safety` / `Support` into the public footer; the DS `BottomNav` primitive (`-17`) shipped with four-tab cores for student (`Match` / `Matches` / `Lessons` / `Messages` + `More` → `Find Tutors` / `Saved` / `Compare`) and tutor (`Overview` / `Lessons` / `Schedule` / `Messages` + `More` → `Profile` / `Credentials` / `Photo` / `Video` / `Earnings`); the desktop single-row guarantee + `MoreMenu` overflow (`-18`) removed every `flex-wrap: wrap` on the top bar so the header stays exactly one row from 768 to 1920 px; the public hamburger drawer (`-19`) gave the public family its own mobile chrome with an optional workspace-shortcut row for signed-in browsers. End-to-end verification under `-20` confirmed: every `AppFrame` consumer (`(public)`, `(student)`, `tutor`, `(account)`, `internal`) passes `viewer`; `auth` and `setup` remain on `FocusedFlowShell` and are unaffected; `app-frame.module.css` carries no `flex-wrap: wrap` on `.headerInner`; the `.desktopNav` hides on `< 768px` via the same media query that reveals the bottom dock; private families render `BottomNav`, the public family renders `PublicMobileMenu`, and the account family inherits the originating role's dock through `loadAccountDockFamily()`. All body-level invariants from `-01` through `-12` still hold: zero hits for `var(--text-*)` / `var(--leading-*)` / `var(--color-text-*)` / `var(--color-border-strong)` / `var(--color-status-positive)` / `var(--color-accent)` under `src/app/(student)`, `src/app/tutor`, `src/components`, or `src/modules`; zero Unicode `★`/`☆` character glyphs anywhere in `src/app`; zero inline `style={{ color: ... }}` on student text rendering paths. Automated verification: `pnpm lint`, `pnpm typecheck`, `pnpm build`, `pnpm lint:arch`, `pnpm test`, and `pnpm test:e2e` (11/11 public-smoke specs passing against `pnpm start`). Known caveats: the bottom-dock student labels (`Match` / `Matches`) are shorter than the top-nav labels (`Get Matched` / `My matches`) — a deliberate fit-the-slot call per the implementing-agent discretion `-17` reserved (`agent-ui-rules.md` § 7); and `(public)/_seo-landing/seo-landing.module.css` still consumes the undefined token vocabulary (`--color-text-muted`, `--color-text`, `--color-text-secondary`, `--color-border`) that `-01` migrated for `lesson-detail.module.css` but did not scope to the public SEO surface — out of scope for this pack, worth a follow-on token-migration task. The Account-family pack (`-21` … `-28`) remains open in this doc.
