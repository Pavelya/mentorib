...
| 5 | `P2-DISPUTE-001` | `draft` | `P2` | 3 | Lesson-issue internal review and dispute resolution surface — **superseded/unbuilt**, now owned by `P2-OPSFIX-006` in the admin task pack (see footnote) |
...
>
> **Footnote — cross-family header pack (second wave).** The header pack inside [`P2-SUX-001`](./phase2-student-ux-task-pack-v1.md) (subtasks `-14` … `-20`) landed end-to-end on 2026-05-26. It ratified the canonical `AppHeader` shape ([app-header-shape-v1 ADR](../architecture/app-header-shape-v1.md)), loaded `viewer` on every `AppFrame` consumer (including the public family), replaced the avatar `<Link>` with the DS `AvatarMenu` popover so account excursions no longer swap the active family's nav rail, introduced the DS `BottomNav` primitive for student and tutor mobile cores (four tabs + More, with the account family inheriting the originating role's dock), guaranteed a single-row desktop header via a `MoreMenu` overflow composition, and shipped the public-family hamburger drawer for `< 768px`. The same DS `Menu` + `Popover` composition now backs the avatar menu, the bottom-dock `More` slot, the desktop-overflow `More` button, and the public hamburger drawer — one composition for four call sites. No new vendors or reference data were introduced. The Account-family pack (subtasks `-21` … `-28`) remains open in the same doc.
>
> **Footnote — Account-family pack (third wave).** The Account-family pack inside [`P2-SUX-001`](./phase2-student-ux-task-pack-v1.md) (subtasks `-21` … `-28`) landed end-to-end on 2026-05-26. It ratified the canonical Account-family IA ([account-family-shape-v1 ADR](../architecture/account-family-shape-v1.md)) — four routes, one job each, role-gated bodies on `/notifications` and `/billing` with the nav rail and avatar menu staying flat. Notification preferences moved from `/settings` to a new `Preferences` tab on `/notifications` (the inbox stays the default tab); a static `NOTIFICATION_CATEGORY_AUDIENCE` lookup in `src/modules/notifications/constants.ts` filters categories so a student-only account sees two, a tutor-only sees three, and a dual-role sees four; the `lesson_reminders` description and the `lesson_report_shared` / `review_submitted` Inbox labels now adapt per active role; operator vocabulary (`Bell inbox`, `Channel rule`, `No chat replay`, `Visible product updates`, `Privacy surface scope`, `Current notice state`, `Deferred controls`, `Scope guardrails`, `Billing history only`) is gone from every Account body. `/privacy` became a real legal-acknowledgement surface — pending-notice banner, current-notice Panel, a `Policy documents` link section to `/privacy-policy` / `/terms` / `/trust-and-safety`, and the published-history list (no metric grid, no future-feature placeholders). `/billing` branches on `hasRole(account, "student")`: tutor-only accounts see one Earnings-CTA Panel; students see the simplified history with renamed metric labels (`Total paid` / `Awaiting confirmation` / `Refunds`). `/settings` reduced to one Profile Panel with a per-role `Lesson language` description, prettier timezone label via `getTimezoneLabel`, helper text on the Email and Timezone Sections, and a one-line `Manage notification preferences →` link below the form. No new vendors, reference-data tables, or DB columns were introduced. Final verification ran under `P2-SUX-001-28`. `P2-SUX-001` is now closed.
>
> **Footnote — admin remediation + expansion pack (2026-05-28).** A 2026-05-28 operator review found that although the `P2-OPS-*` admin domain layer works, the shipped screens are unusable (raw UUIDs/enums, no avatars/tutor info, ~700 lines of route-local CSS), and `P2-DISPUTE-001` was never built (no `/internal/disputes` route). The diagnosis and decisions live in [`phase2-admin-remediation-plan-v1.md`](./phase2-admin-remediation-plan-v1.md); the expected end-state capability catalog + navigation IA live in [`phase2-admin-capability-map-v1.md`](./phase2-admin-capability-map-v1.md); and the executable tasks live in the dedicated [`phase2-admin-task-pack-v1.md`](./phase2-admin-task-pack-v1.md) — 16 tasks: 7 legibility fixes (`P2-OPSFIX-001` … `-006a`, including the missing dispute build `P2-OPSFIX-006`) and 9 expansion builds (`P2-ADMIN-NAV-001`, `PEOPLE-001/002`, `TRUST-001`, `POLICY-001`, `SUPPORT-001`, `ANALYTICS-001`, `FIN-000/001`). Key decisions ratified there: admin DTOs are view models (no raw ids/enums), all people render via `PersonSummary`, navigation is two-level on the shared `AppFrame` header (no admin sidebar), legal text stays code-managed (admin manages versions + broadcasts only), and the revenue model is a single admin-configured global commission % (the prerequisite for all platform finance reporting). `P2-DISPUTE-001` above is reset to `draft` and superseded by `P2-OPSFIX-006`.
...
**Status:** `draft` — **NOT built.** This task was previously marked done, but no `/internal/disputes` route exists in the repo. The detailed scope below remains the canonical spec, but execution is now owned by **`P2-OPSFIX-006`** in `docs/planning/phase2-admin-task-pack-v1.md`, which builds it on the remediated view-model boundary + DS chrome so it does not repeat the legibility failures of the other admin surfaces. Do not implement this entry directly; pick `P2-OPSFIX-006`.
**Priority:** `P2`
**Wave:** 3
